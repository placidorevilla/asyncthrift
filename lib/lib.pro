...
java_dep.output = $$TOP_BUILDDIR/target/$$PKGLIBDIR/java/${QMAKE_FILE_IN_BASE}
...
for(file, JAVA_DEPS) {
	dir = $$dirname(file)
	name = $$replace(dir, "[./\\\\]", "_")
	file = $$TOP_BUILDDIR/target/$$PKGLIBDIR/$$replace(file, "(.*)\\.md5$", "\\1")
	eval($${name}.path = $$PREFIX$$PKGLIBDIR/java/)
