...
JAVA_TARGET = $$TOP_BUILDDIR/target/$$PKGLIBDIR/java/suasync-generic-callback.jar

java.input = JAVA_SOURCES
java.output = $$JAVA_TARGET
java.commands = $${QMAKE_MKDIR} java && $$JAVAC -cp $$TOP_BUILDDIR/target/$$PKGLIBDIR/java/\'*\' -d java ${QMAKE_FILE_IN} && jar cvf ${QMAKE_FILE_OUT} -C java `ls java` || (rm ${QMAKE_FILE_OUT} && false)
...
java_install.path = $$PREFIX$$PKGLIBDIR/java/
